...
void MeshRefiner::genNormalsWithSmoothAngle(float smooth_angle, bool flip)
{
    ...
    for (size_t fi = 0; fi < num_faces; ++fi)
    {
        ...
        for (int ci = 0; ci < count; ++ci) {
            ...
            int num_connections = connection.counts[vi];
            int *cfaces = &connection.faces[connection.offsets[vi]];
...
void MeshRefiner::buildConnection()
{
    ...
    BuildVerticesConnection(indices, counts, points.size(), connection);
}
...
int MeshRefiner::findOrAddVertexPNTUC(int vi, const float3& p, const float3& n, const float4& t, const float2& u, const float4& c)
{
    ...
    int count = connection.counts[vi];
    for (int ci = 0; ci < count; ++ci) {
        int& ni = old2new[connection.indices[offset + ci]];
