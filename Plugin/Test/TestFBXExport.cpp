...
void TestFBXExport()
{
    ...
    std::vector<float3> points;
    std::vector<float2> uv;
    GenerateWaveMesh(counts, indices, points, uv, 1.0f, 0.25f, 128, 0.0f, true);
    auto mesh = ctx->addMesh(parent, "Mesh", float3::zero(), quatf::identity(), float3::one(),
        indices.size() / 3, points.size(), indices.data(), points.data(), nullptr, nullptr, uv.data(), nullptr);

    ctx->write("test_binary.fbx", false);
    ctx->write("test_ascii.fbx", true);
    ctx->release();
#endif
}
